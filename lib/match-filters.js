...
function groupMatchesByWeek(matches) {
  ...
  for (const match of matches) {
    ...
    if (!currentGroup || currentGroup.week !== match.week) {
      groups.push({ week: match.week, matches: [match] });
    } else {
...
function getDefaultOpenWeek(matches, selectedWeek) {
  if (Number.isInteger(selectedWeek)) return selectedWeek;

  const firstOpenMatch = matches.find((match) => match.status !== 'played');
  if (firstOpenMatch) return firstOpenMatch.week;

  return matches.length ? matches[matches.length - 1].week : null;
}
